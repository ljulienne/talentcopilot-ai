...
def test_upload_result_becomes_official_recruitment_session():
    ...
    assert [item.decision_score for item in session.ranked_analyses] == [84.0, 24.0]
    assert [item.official_match_score for item in session.ranked_analyses] == [82.5, 21.0]
    assert [item.official_decision_score for item in session.ranked_analyses] == [84.0, 24.0]
...
def test_zero_official_fit_is_preserved():
    ...
    assert david.official_match_score == 0
    assert david.decision_score == 41
    assert david.official_decision_score == 41
    assert david.rank == 2
