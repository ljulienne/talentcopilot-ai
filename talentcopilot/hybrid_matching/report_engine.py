...
class HybridRecruiterReportEngine:
    ...
    def attach_interview_questions(self, hybrid_report):
        if hybrid_report.recruiter_report:
            plan = HybridInterviewFocusEngine().build(hybrid_report)
            hybrid_report.recruiter_report.structured_interview_questions = plan.questions
    ...
    def _interview_focus(self, report, gaps: list[str]) -> list[str]:
        focus = [f"Validate practical experience with {gap}." for gap in gaps]
