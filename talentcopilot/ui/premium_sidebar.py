...
def resolve_recruitment_destinations(session, *, current_page: str = "") -> tuple[SidebarItem, ...]:
    ...
    overview_target = "Recruitment Overview" if session is not None and analyzed else "Recruitment Workspace"
    ...
    return (
        SidebarItem(
            ...
            "Overview",
            overview_target,
            "Mission dashboard and recommended next action.",
            "▦",
        ...
        SidebarItem(
            ...
            "Review candidate fit, competencies and grounded evidence.",
            "◇",
...
def render_premium_sidebar(session, *, current_page: str, app_version: str = "") -> None:
    ...
    for item in resolve_recruitment_destinations(session, current_page=current_page):
    ...
    secondary = (
        SidebarItem(
            ...
            "Explore organizational and collaboration signals.",
            "⌘",
        ),
        SidebarItem(
            "analytics",
            "Analytics",
            "Analytics Dashboard",
            "Open cross-recruitment analytics.",
            "⌁",
        ),
    )
    ...
    with st.sidebar.expander("More", expanded=False):
        more = (
            SidebarItem("home", "Executive brief", "Executive Brief", "Return to the decision-oriented home.", "⌂"),
            ...
            SidebarItem("budget", "Hiring budget", "Hiring Budget", "Review the hiring budget workspace.", "¤"),
        )
        for item in more:
            if st.button(
                item.label,
    ...
    context = get_workflow_context(session, current_page=current_page)
    next_label = "Review candidates"
    next_page = "Candidate Intelligence"
    if not session:
        next_label, next_page = "Start recruitment", "Recruitment Workspace"
    elif not context.interview_prepared_candidate_ids:
        next_label, next_page = "Prepare an interview", "Interview Intelligence"
    elif context.interview_assessed_candidate_ids and not context.finalists_compared:
