...
class RecruitmentUploadSessionService:
    ...
    def _candidate_dict(self, name: str, document: Any, ranked: Any) -> dict:
        ...
        evidence = self._comparative_evidence(profile) or self._mission_fit_evidence(profile) or self._profile_evidence(profile)
        if not evidence:
    ...
    def _comparative_evidence(self, profile: Any) -> List[str]:
        """Return candidate-specific differentiators before generic evidence."""
        metadata = getattr(profile, "metadata", {}) or {}
        raw = metadata.get("comparative_differentiators")
        if not raw:
            return []
        try:
    ...
    def _score_breakdown(
        ...
        *,
        ...
        comparative_score = metadata.get("comparative_score")
        ...
        if raw_comparative:
            try:
                comparative = json.loads(raw_comparative) if isinstance(raw_comparative, str) else raw_comparative
                for key in ("role_level", "geographic_scope", "leadership_scope", "commercial_scope", "industry_depth", "career_progression"):
                    if key in comparative:
                        breakdown[f"comparative_{key}"] = round(self._number(comparative[key]), 2)
            except (TypeError, ValueError, json.JSONDecodeError):
                pass
